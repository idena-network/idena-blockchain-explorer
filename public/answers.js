...
function updateAnswersData(data, table){
  ...
  for (var i = 0; i < data.result.length; i++) {
        ...
        var td=$("<td/>");
            ...
            td.append("<div class='text_block text_block--ellipsis'><a href='./flip?flip="+cid+"'>" + cid.substr(0, 35) + "...</a></div>");
        tr.append(td);
        tr.append("<td>"+(data.result[i].flipAnswer=="None"?"-":data.result[i].flipAnswer)+"</td>");

        var score="0";
        var ico=(data.result[i].respAnswer=='None'?'':'icon--cancel');

        if (data.result[i].flipStatus=="Qualified"){
          if (data.result[i].flipAnswer==data.result[i].respAnswer){
            score=1;
            ico='icon--micro_check'; 
          }
        } else {
        ...
        tr.append("<td>"+ (data.result[i].flipStatus=="Qualified"?"Strong": (data.result[i].flipStatus=="WeaklyQualified"?"Weak":"No consensus")) +"</td>");

        var td=$("<td/>");
            td.append("<i class='icon "+ico+"'>"); 
            td.append("<span>"+(data.result[i].respAnswer=="None"?"-":data.result[i].respAnswer)+"</span>");
        tr.append(td);
...
function updateIdentityAnswersData(data){
  ...
  if (data.result.shortAnswers.flipsCount>0)
    $("#IdentityShortScore")[0].textContent=precise2(data.result.shortAnswers.point / data.result.shortAnswers.flipsCount * 100)+'%';

  $("#IdentityLongAnswers")[0].textContent=data.result.longAnswers.point +' / '+data.result.longAnswers.flipsCount;
  if (data.result.longAnswers.flipsCount>0)
    $("#IdentityLongScore")[0].textContent=precise2(data.result.longAnswers.point / data.result.longAnswers.flipsCount * 100)+'%';
